...
void DegreeSqSumEstimatorUtil (CGraph *cg, Parameters params, int c) {
    OrderedEdgeCollection randomEdgeCollection;
    std::vector<Estimates> estimates;
    // Set up random number generator
    std::random_device rd;
    ...
    VertexIdx wedge_count = 0;
    for (VertexIdx src=0; src < cg->nVertices; src++) {
        VertexIdx deg = cg->degree(src);
        wedge_count += deg * (deg-1) /2;
    }
    EstimatorStats est_stats = GetErrorStatistics(estimates, wedge_count);

    // Print to console
    if (params.print_to_console) {
        WriteHeaderInOutput(stdout, params, cg, wedge_count);
        WriteAlgorithmOutput(stdout, params.algo_name, params, est_stats);
        WriteRawData(stdout,estimates);
    }
...
void DegreeSqEstimator (CGraph *cg, Parameters params) {
    std::vector<int> c = {10};
    for (auto item : c) {
        DegreeSqSumEstimatorUtil(cg, params, item);
    }
}
